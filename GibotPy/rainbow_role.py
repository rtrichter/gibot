...
class Rainbow(commands.Cog):
    def __init__(self, bot, guild):
        self.bot = bot
        self.guild = guild
        self.rainbow_role_update.start()

    # initialize the rainbow role update loop
    @tasks.loop(seconds=constants.get_config("rainbow_role_update_period_s"))
    async def rainbow_role_update(self):
        ...
        rainbow_role = constants.get_role(self.guild, "rainbow")
        # set the color of the rainbow role
        await rainbow_role.edit(color=new_color)

    async def rainbow_role_update_loop(self):
        # wait until the bot is ready
        await bot.bot.wait_until_ready()

        await self.rainbow_role_update()
        ...
        await self.rainbow_role_update_loop() 